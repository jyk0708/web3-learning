...
contract MultiSigWalletUpgradeable is Initializable, ContextUpgradeable, UUPSUpgradeable {
    ...
    function executeTx(uint256 _txIndex) external onlyOwner txExists(_txIndex) notExecuted(_txIndex) {
        ...
        // 执行交易提案（无论 value 是否为 0，都要执行 call）
     ...
     */
    function createUpgradeProposal(address _newImplementation) external onlyOwner returns (uint256) {
     ...
     */
    function confirmUpgradeProposal(uint256 _proposalId) external onlyOwner {
     ...
     * @param _proposalId 升级提案的索引
     */
    function revokeUpgradeConfirmation(uint256 _proposalId) external onlyOwner {
        require(_proposalId < upgradeProposals.length, "Upgrade proposal not found");
        UpgradeProposal storage proposal = upgradeProposals[_proposalId];
        require(!proposal.executed, "Proposal already executed");
     ...
     * @param _proposalId 升级提案的索引
     */
    function executeUpgradeProposal(uint256 _proposalId) external onlyOwner {
        require(_proposalId < upgradeProposals.length, "Upgrade proposal not found");
        UpgradeProposal storage proposal = upgradeProposals[_proposalId];
        require(!proposal.executed, "Proposal already executed");
        ...
        // upgradeTo 是 external 函数，通过低级 call 触发外部调用，经过代理 delegatecall
     ...
     */
    function getUpgradeProposal(uint256 _proposalId) external view returns (
        address newImplementation,
        uint256 confirmations,
        bool executed
    ) {
     ...
     * @return 是否已确认
     */
    function isUpgradeConfirmed(uint256 _proposalId, address _confirmor) external view returns (bool) {
